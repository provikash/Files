...
async def add_premium_user(user_id: int, plan_type: str, tokens: int):
    ...
    # If it's unlimited plan, add expiry date (1 year default)
    if tokens == -1:
        premium_user['expiry_date'] = datetime.utcnow() + timedelta(days=365)
    
    await premium_data.replace_one({'_id': user_id}, premium_user, upsert=True)
