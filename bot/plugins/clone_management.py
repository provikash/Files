...
async def start_clone_process(bot_info: dict):
    ...
    try:
        ...
        asyncio.create_task(run_clone_bot(clone_bot))
        
    except Exception as e:
        print(f"Error starting clone process: {e}")

class BotClone(Client):
    """Individual bot clone class"""
    def __init__(self, name, api_id, api_hash, bot_token, owner_id):
        super().__init__(
            name=name,
            api_id=api_id,
            api_hash=api_hash,
    ...
    async def start(self):
        ...
        print(f"✅ Clone bot @{me.username} started successfully!")

async def run_clone_bot(clone_bot: BotClone):
    """Run a clone bot"""
    try:
        ...
        print(f"Error running clone bot: {e}")
    finally:
